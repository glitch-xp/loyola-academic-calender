...
// .wrangler/tmp/bundle-2se4pg/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-2se4pg/middleware-insertion-facade.js
init_modules_watch_stub();
...
var Context = class {
  ...
  #newResponse(data2, arg, headers2) {
   ...
   * ```
   */
  body = /* @__PURE__ */ __name((data2, arg, headers2) => this.#newResponse(data2, arg, headers2), "body");
  /**
   ...
   * ```
   */
  text = /* @__PURE__ */ __name((text3, arg, headers2) => {
    return !this.#preparedHeaders && !this.#status && !arg && !headers2 && !this.finalized ? new Response(text3) : this.#newResponse(
      text3,
      arg,
  ...
  html = /* @__PURE__ */ __name((html3, arg, headers2) => {
...
config.get("/", async (c) => {
  ...
  try {
    const [departments, shifts, shiftTimings, timetables] = await Promise.all([
      ...
      db.prepare("SELECT * FROM shift_timings ORDER BY shift_id, period").all(),
      db.prepare("SELECT * FROM timetables ORDER BY department_id, year, shift_id, section").all()
    ]);
    const shiftsConfig = shifts.results.map((shift) => ({
      id: shift.id,
      name: shift.name,
      timings: shiftTimings.results.filter((t) => t.shift_id === shift.id).map((t) => ({
        period: t.period,
    ...
    const departmentsConfig = departments.results.map((dept) => {
      ...
      const yearGroups = /* @__PURE__ */ new Map();
      ...
      const years = Array.from(yearGroups.entries()).map(([year, yearTTs]) => {
        ...
        if (hasShifts) {
          const shiftGroups = /* @__PURE__ */ new Map();
          ...
          const shiftDetails = Array.from(shiftGroups.entries()).map(
            ([shiftId, shiftTTs]) => {
              if (hasSections && shiftTTs.some((t) => t.section)) {
                return {
                  shiftId,
                  contributor: shiftTTs[0]?.contributor || void 0,
                  sections: shiftTTs.filter((t) => t.section).map((t) => ({
                    name: t.section,
                    timetableId: t.id,
                    contributor: t.contributor || void 0
                  }))
                };
              }
              return {
          ...
          const uniqueShifts = [...new Set(yearTTs.map((t) => t.shift_id).filter(Boolean))];
          return {
            year,
            shifts: uniqueShifts,
            shiftDetails
        ...
        return {
          ...
          contributor: yearTTs[0]?.contributor || void 0
        };
      });
      return {
        name: dept.name,
...
calendar.get("/", async (c) => {
  const db = c.env.DB;
  try {
    const result = await db.prepare("SELECT * FROM calendar_days ORDER BY date").all();
    const calendarData = {};
...
init_modules_watch_stub();
var timetable = new Hono2();
timetable.get("/:id", async (c) => {
  const timetableId = c.req.param("id");
  const db = c.env.DB;
  try {
    const ttExists = await db.prepare("SELECT id FROM timetables WHERE id = ?").bind(timetableId).first();
    if (!ttExists) {
...
version.get("/", async (c) => {
  ...
  try {
    ...
    const isUpdateAvailable = currentVersion ? compareVersions(latest.version, currentVersion) > 0 : false;
    return c.json({
...
init_modules_watch_stub();
async function authMiddleware(c, next2) {
...
admin.get("/departments", async (c) => {
  const result = await c.env.DB.prepare("SELECT * FROM departments ORDER BY sort_order, name").all();
...
admin.post("/departments", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO departments (id, name, sort_order) VALUES (?, ?, ?)").bind(id, name, sort_order || 0).run();
  return c.json({ success: true, id });
});
admin.put("/departments/:id", async (c) => {
  const id = c.req.param("id");
  const { name, sort_order } = await c.req.json();
  await c.env.DB.prepare("UPDATE departments SET name = ?, sort_order = ?, updated_at = datetime('now') WHERE id = ?").bind(name, sort_order || 0, id).run();
...
admin.get("/shifts", async (c) => {
  ...
  const result = shifts.results.map((shift) => ({
    ...shift,
    timings: timings.results.filter((t) => t.shift_id === shift.id)
  }));
...
admin.post("/shifts", async (c) => {
  ...
  if (timings && Array.isArray(timings)) {
    for (const t of timings) {
      await c.env.DB.prepare("INSERT INTO shift_timings (shift_id, period, start_time, end_time) VALUES (?, ?, ?, ?)").bind(id, t.period, t.startTime || t.start_time, t.endTime || t.end_time).run();
    }
  }
  return c.json({ success: true, id });
});
...
admin.post("/timetables", async (c) => {
  ...
  ).bind(id, department_id, year, shift_id || null, section || null, contributor || null).run();
  return c.json({ success: true, id });
});
admin.put("/timetables/:id", async (c) => {
  ...
  ).bind(department_id, year, shift_id || null, section || null, contributor || null, id).run();
  return c.json({ success: true });
});
...
admin.get("/timetable-entries/:timetableId", async (c) => {
  ...
  const result = await c.env.DB.prepare("SELECT * FROM timetable_entries WHERE timetable_id = ? ORDER BY day_order, period").bind(timetableId).all();
  return c.json(result.results);
});
admin.post("/timetable-entries", async (c) => {
  ...
  ).bind(timetable_id, day_order, period, subject_name || "", subject_code || "", room || "", teacher || "").run();
...
admin.put("/timetable-entries/:id", async (c) => {
  ...
  ).bind(subject_name || "", subject_code || "", room || "", teacher || "", id).run();
  return c.json({ success: true });
});
...
admin.post("/timetable-entries/bulk/:timetableId", async (c) => {
  ...
  await c.env.DB.prepare("DELETE FROM timetable_entries WHERE timetable_id = ?").bind(timetableId).run();
  ...
  await c.env.DB.prepare("UPDATE timetables SET updated_at = datetime('now') WHERE id = ?").bind(timetableId).run();
...
admin.post("/calendar", async (c) => {
  ...
  ).bind(date, day_order || null, is_holiday ? 1 : 0, event_title || null, event_description || null).run();
  return c.json({ success: true });
});
...
admin.get("/releases", async (c) => {
  const result = await c.env.DB.prepare("SELECT * FROM app_releases ORDER BY created_at DESC").all();
  return c.json(result.results);
});
admin.post("/releases", async (c) => {
  const { version: version2, platform, release_notes, download_url, is_latest } = await c.req.json();
  if (!version2 || !platform) {
    return c.json({ error: "version and platform are required" }, 400);
  }
  if (is_latest) {
    await c.env.DB.prepare("UPDATE app_releases SET is_latest = 0 WHERE platform = ?").bind(platform).run();
  }
  const result = await c.env.DB.prepare(
    "INSERT INTO app_releases (version, platform, release_notes, download_url, is_latest) VALUES (?, ?, ?, ?, ?)"
  ).bind(version2, platform, release_notes || null, download_url || null, is_latest ? 1 : 0).run();
  return c.json({ success: true, id: result.meta.last_row_id });
});
admin.put("/releases/:id", async (c) => {
  ...
  ).bind(version2, platform, release_notes || null, download_url || null, is_latest ? 1 : 0, id).run();
  return c.json({ success: true });
});
...
admin.post("/change-password", async (c) => {
  ...
  const user = await c.env.DB.prepare("SELECT * FROM admin_users WHERE id = ?").bind(adminUser.sub).first();
  if (!user) return c.json({ error: "User not found" }, 404);
  const valid = await verifyPassword(currentPassword, user.password_hash);
  if (!valid) return c.json({ error: "Current password is incorrect" }, 401);
  const newHash = await hashPassword(newPassword);
  await c.env.DB.prepare("UPDATE admin_users SET password_hash = ? WHERE id = ?").bind(newHash, user.id).run();
  return c.json({ success: true });
});
...
admin.get("/contributions/:id", async (c) => {
  ...
  const existingTT = await db.prepare(`
    ...
    contribution.year,
    contribution.shift_id,
    contribution.shift_id,
    contribution.section,
    contribution.section
  ).first();
  if (existingTT) {
    const entries = await db.prepare("SELECT * FROM timetable_entries WHERE timetable_id = ? ORDER BY day_order, period").bind(existingTT.id).all();
    const timetableData = {};
...
admin.post("/contributions/:id/approve", async (c) => {
  ...
  const db = c.env.DB;
  const adminUser = c.get("adminUser");
  const contribution = await db.prepare("SELECT * FROM pending_contributions WHERE id = ? AND status = ?").bind(id, "pending").first();
  if (!contribution) {
  ...
  const finalTtId = existingTT ? existingTT.id : ttId;
  if (existingTT) {
    await db.prepare("DELETE FROM timetable_entries WHERE timetable_id = ?").bind(finalTtId).run();
    await db.prepare(
...
admin.post("/contributions/:id/reject", async (c) => {
  ...
  const contribution = await db.prepare("SELECT id FROM pending_contributions WHERE id = ? AND status = ?").bind(id, "pending").first();
  if (!contribution) {
    return c.json({ error: "Contribution not found or already reviewed" }, 404);
  }
  await db.prepare(
...
admin.get("/stats", async (c) => {
  const [departments, timetables, calendarDays, releases, pendingContributions] = await Promise.all([
    ...
    c.env.DB.prepare("SELECT COUNT(*) as count FROM timetables").first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM calendar_days").first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM app_releases").first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM pending_contributions WHERE status = 'pending'").first()
  ]);
  return c.json({
...
admin.post("/seed", async (c) => {
  const { masterConfig, calendar: calendar2, timetables } = await c.req.json();
  const db = c.env.DB;
  ...
  try {
      ...
      for (const shift of masterConfig.shifts) {
        ...
        if (shift.timings) {
          ...
          for (const t of shift.timings) {
            await db.prepare("INSERT INTO shift_timings (shift_id, period, start_time, end_time) VALUES (?, ?, ?, ?)").bind(shift.id, t.period, t.startTime, t.endTime).run();
          }
        }
      }
    }
    if (masterConfig?.departments) {
      for (const dept of masterConfig.departments) {
        ...
        await db.prepare("INSERT OR REPLACE INTO departments (id, name, sort_order) VALUES (?, ?, ?)").bind(deptId, dept.name, imported.departments).run();
          ...
          if (year.shiftDetails) {
            for (const sd of year.shiftDetails) {
              if (sd.timetableId) {
                ...
                ).bind(sd.timetableId, deptId, year.year, sd.shiftId || null, null, sd.contributor || null).run();
                imported.timetables++;
              }
              if (sd.sections) {
                for (const sec of sd.sections) {
                  await db.prepare(
                    "INSERT OR REPLACE INTO timetables (id, department_id, year, shift_id, section, contributor) VALUES (?, ?, ?, ?, ?, ?)"
                  ).bind(sec.timetableId, deptId, year.year, sd.shiftId || null, sec.name, sec.contributor || sd.contributor || null).run();
                  imported.timetables++;
                }
              }
            }
            ...
            await db.prepare(
              "INSERT OR REPLACE INTO timetables (id, department_id, year, shift_id, section, contributor) VALUES (?, ?, ?, ?, ?, ?)"
            ).bind(year.timetableId, deptId, year.year, null, null, year.contributor || null).run();
            imported.timetables++;
          }
        }
      }
    ...
    if (timetables) {
      for (const [ttId, ttData] of Object.entries(timetables)) {
        ...
        const data2 = ttData;
        for (const [dayOrder, subjects] of Object.entries(data2)) {
          const dayNum = parseInt(dayOrder);
          ...
          for (let period = 0; period < subjects.length; period++) {
            ...
            await db.prepare(
              "INSERT INTO timetable_entries (timetable_id, day_order, period, subject_name, subject_code, room, teacher) VALUES (?, ?, ?, ?, ?, ?, ?)"
            ).bind(ttId, dayNum, period + 1, sub.name || "", sub.code || "", sub.room || "", sub.teacher || "").run();
            imported.entries++;
          }
        }
      }
    ...
    if (calendar2) {
      for (const [date, config2] of Object.entries(calendar2)) {
        const cfg = config2;
        let dayOrder = null;
        if (cfg.dayOrder) {
          const match2 = String(cfg.dayOrder).match(/(\d+)/);
          dayOrder = match2 ? parseInt(match2[1]) : null;
        }
        await db.prepare(
          "INSERT OR REPLACE INTO calendar_days (date, day_order, is_holiday, event_title, event_description) VALUES (?, ?, ?, ?, ?)"
        ).bind(date, dayOrder, cfg.isHoliday ? 1 : 0, cfg.event || null, cfg.description || null).run();
...
function isSafeAlphanumeric(str, maxLen) {
  if (str.length > maxLen) return false;
  return /^[a-zA-Z0-9\s\-_.]*$/.test(str);
}
__name(isSafeAlphanumeric, "isSafeAlphanumeric");
function validateAndSanitizeTimetable(data2) {
  ...
  for (const dayOrder of validDayOrders) {
    ...
    if (dayData === void 0 || dayData === null) {
      result[dayOrder] = [];
      continue;
    }
...
__name(validateAndSanitizeTimetable, "validateAndSanitizeTimetable");
contributions.post("/", async (c) => {
  ...
  ).bind(clientIp, oneHourAgo).first();
  ...
  const deptExists = await db.prepare("SELECT id FROM departments WHERE id = ?").bind(cleanDeptId).first();
  if (!deptExists) {
  ...
  let cleanShiftId = null;
  if (shift_id !== void 0 && shift_id !== null && shift_id !== "") {
    if (typeof shift_id !== "string") {
      return c.json({ error: "shift_id must be a string" }, 400);
    }
    cleanShiftId = stripHtml(shift_id).substring(0, 20);
    const shiftExists = await db.prepare("SELECT id FROM shifts WHERE id = ?").bind(cleanShiftId).first();
    if (!shiftExists) {
      return c.json({ error: "Invalid shift" }, 400);
    }
  }
  let cleanSection = null;
  if (section !== void 0 && section !== null && section !== "") {
  ...
  let cleanContributor = null;
  if (contributor_name !== void 0 && contributor_name !== null && contributor_name !== "") {
  ...
  const existingPending = await db.prepare(
    `SELECT id FROM pending_contributions 
             ...
             AND (section = ? OR (section IS NULL AND ? IS NULL))
             AND status = 'pending'`
  ).bind(cleanDeptId, year, cleanShiftId, cleanShiftId, cleanSection, cleanSection).first();
  if (existingPending) {
    return c.json({
...
contributions.get("/check", async (c) => {
  ...
  const existing = await db.prepare(
    `SELECT id, status, created_at FROM pending_contributions 
             ...
             AND status = 'pending'
             LIMIT 1`
  ).bind(deptId, year, shiftId, shiftId, section, section).first();
  return c.json({
    hasPending: !!existing,
    submission: existing ? { id: existing.id, created_at: existing.created_at } : null
  });
});
var contributions_default = contributions;
  ...
  for (let i = 0; i < 12; i++) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const $2 = load(html3);
        $2(".events-list .event-item").each((_, el) => {
          const dayStr = $2(el).find(".event-day").text().trim();
          const dayOrderStr = $2(el).find(".event-day-order").text().trim();
          const eventText = $2(el).find(".event-text").text().trim();
          const dayName = $2(el).find(".event-day-name").text().trim();
          if (!dayStr) return;
...
__name(scrapeCalendar, "scrapeCalendar");
...
app.use(
  ...
  cors({
    origin: ["*"],
    // In production, restrict to your Cloudflare Pages domains
...
app.route("/api/config", config_default);
app.route("/api/calendar", calendar_default);
app.route("/api/timetable", timetable_default);
app.route("/api/version", version_default);
app.route("/api/contributions", contributions_default);
